 ...
 * 
 * @author spruce */
public class RavChainLight extends Light {
	 ...
	 *           </ul>
	*/
	public RavChainLight (RayHandler rayHandler, int rays, Color color, float distance, int rayDirection) {
	 ...
	 * 
	 * @param body that will be automatically followed, note that the body rotation angle is taken into account for the light
	 *           offset and direction calculations
	...
	/** Calculates ray positions and angles along chain. This should be called any time the number or values of elements changes in
	 * {@link #chain}. */
	...
	void applyAttachment () {
		restorePosition.setToTranslation(getPosition());
		rotateAroundZero.setToRotation(getDirection());
		for (int i = 0; i < rayNum; i++) {
	...
	protected void updateComponent () {
		final Vector2 vec = super.getPosition();
		tmpVec.set(0, 0).sub(super.getPosition());
		bodyPosition.set(vec);
		zeroPosition.setToTranslation(tmpVec);
		restorePosition.setToTranslation(super.getPosition());
		rotateAroundZero.setToRotation(getDirection()).inv().rotate(getDirection());
