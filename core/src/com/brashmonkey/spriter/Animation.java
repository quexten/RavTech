...
/** Represents an animation of a Spriter SCML file. An animation holds {@link Timeline}s and a {@link Mainline} to animate
 * objects. Furthermore it holds an {@link #id}, a {@link #length}, a {@link #name} and whether it is {@link #looping} or not.
 * 
 * @author Trixt0r */
public class Animation {
	...
	public String toString () {
		String toReturn = getClass().getSimpleName() + "|[id: " + id + ", " + name + ", duration: " + length + ", is looping: "
			+ looping;
		toReturn += "Mainline:\n";
		toReturn += mainline;
		toReturn += "Timelines\n";
	...
	protected void update (BoneRef ref, Bone root, int time) {
		...
		this.unmapTimelineObject(ref.timeline, isObject,
	...
	void unmapTimelineObject (int timeline, boolean isObject, Bone root) {
		...
		if (isObject)
			((Object)mapTarget).set((Object)tweenTarget);
		else
	...
	/*
	 * Timeline getSimilarTimeline(BoneRef ref, Collection<Timeline> coveredTimelines){ if(ref.parent == null) return null;
	 * for(BoneRef boneRef: this.currentKey.objectRefs){ Timeline t = this.getTimeline(boneRef.timeline); if(boneRef.parent != null
	 * && boneRef.parent.id == ref.parent.id && !coveredTimelines.contains(t)) return t; } return null; }
	 * 
	 * Timeline getSimilarTimeline(ObjectRef ref, Collection<Timeline> coveredTimelines){ if(ref.parent == null) return null;
	 * for(ObjectRef objRef: this.currentKey.objectRefs){ Timeline t = this.getTimeline(objRef.timeline); if(objRef.parent != null
	 * && objRef.parent.id == ref.parent.id && !coveredTimelines.contains(t)) return t; } return null; }
	 */

	/** Prepares this animation to set this animation in any time state. This method has to be called before
	 * {@link #update(int, Bone)}. */
	public void prepare () {
		if (this.prepared)
			return;
		...
		for (int i = 0; i < this.tweenedKeys.length; i++) {
			...
			this.tweenedKeys[i].setObject(new Timeline.Key.Object(new Vector2(0, 0)));
			this.unmappedTweenedKeys[i].setObject(new Timeline.Key.Object(new Vector2(0, 0)));
		}
