...
/** An inverse kinematics resolver implementation. An instance of this class uses the CCD (Cyclic Coordinate Descent) algorithm to
 * resolve the constraints.
 * 
 * @see <a href="https://sites.google.com/site/auraliusproject/ccd-algorithm"> ccd-algorithm</a> and
 *      <a href= "http://www.ryanjuckett.com/programming/cyclic-coordinate-descent-in-2d/"> cyclic-coordinate-descent-in-2d</a> .
 * @author Trixt0r */
public class CCDResolver extends IKResolver {
	...
	public void resolve (float x, float y, int chainLength, BoneRef effectorRef) {
		...
		if (Math.signum(player.root.scale.x) == -1)
			effector.angle += 180f;
		...
		for (int i = 0; i < chainLength && parentRef != null; i++) {
			if (Calculator.distanceBetween(xx, yy, x, y) <= this.tolerance)
				return;
			parent.angle += Calculator.angleDifference(
