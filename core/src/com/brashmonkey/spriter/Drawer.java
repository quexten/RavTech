...
/** A Drawer is responsible for drawing a {@link Player}. Since this library is meant to be as generic as possible this class has
 * to be abstract, because it cannot be assumed how to draw a resource. Anyone who wants to draw a {@link Player} has to know how
 * to draw a resource. A resource can be e.g. a sprite, a texture or a texture region. To draw a {@link Player} call
 * {@link #draw(Player)}. This method relies on {@link #draw(Object)}, which has to be implemented with the chosen backend. To
 * debug draw a {@link Player} call {@link #drawBones(Player)}, {@link #drawBoxes(Player)} and {@link #drawPoints(Player)}, which
 * rely on {@link #rectangle(float, float, float, float)}, {@link #circle(float, float, float)},
 * {@link #line(float, float, float, float)} and {@link #setColor(float, float, float, float)}.
 * 
 * @author Trixt0r
 *
...
public abstract class Drawer<R> {
	...
	public void drawBones (Player player) {
		...
		/*
		 * for(Mainline.Key.BoneRef ref: player.getCurrentKey().boneRefs){ Timeline.Key key =
		 * player.unmappedTweenedKeys[ref.timeline]; Timeline.Key.Bone bone = key.object();
		 * if(player.animation.getTimeline(ref.timeline).objectInfo.type != ObjectType.Bone || !key.active) continue; ObjectInfo
		 * info = player.animation.getTimeline(ref.timeline).objectInfo; if(info == null) continue; Dimension size = info.size;
		 * drawBone(bone, size); }
		 */
	}
