...
/** Represents a mainline in a Spriter SCML file. A mainline holds only keys and occurs only once in an animation. The mainline is
 * responsible for telling which draw order the sprites have and how the objects are related to each other, i.e. which bone is the
 * root and which objects are the children.
 * 
 * @author Trixt0r */
public class Mainline {
	...
	/** Represents a mainline key in a Spriter SCML file. A mainline key holds an {@link #id}, a {@link #time}, a {@link #curve}
	...
	public static class Key {
		 ...
		 * @return the bone reference with the given time line index or null if no reference exists with the given time line
		 *         index */
		 ...
		 * 
		 * @param timeline the time line index
		 * @return the object reference with the given time line index or null if no reference exists with the given time line
		 *         index */
		...
		/** Represents a bone reference in a Spriter SCML file. A bone reference holds an {@link #id}, a {@link #timeline} and a
		...
		public static class BoneRef {
			...
			public String toString () {
				...
				return getClass().getSimpleName() + "|id: " + id + ", parent:" + parentId + ", timeline: " + timeline + ", key: "
					+ key;
			}
		}

		/** Represents an object reference in a Spriter SCML file. An object reference extends a {@link BoneRef} with a
		 * {@link #zIndex}, which indicates when the object has to be drawn.
		 * 
		 * @author Trixt0r */
