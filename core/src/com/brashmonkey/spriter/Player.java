...
/** A Player instance is responsible for updating an {@link Animation} properly. With the {@link #update()} method an instance of
 * this class will increase its current time and update the current set animation ( {@link #setAnimation(Animation)}). A Player
 * can be positioned with {@link #setPivot(float, float)}, scaled with {@link #setScale(float)}, flipped with
 * {@link #flip(boolean, boolean)} and rotated {@link #setAngle(float)}. A Player has various methods for runtime object
 * manipulation such as {@link #setBone(String, Bone)} or {@link #setObject(String, Bone)}. Events like the ending of an animation
 * can be observed with the {@link PlayerListener} interface. Character maps can be changed on the fly, just by assigning a
 * character maps to {@link #characterMaps}, setting it to <code>null</code> will remove the current character map.
 * 
 * @author Trixt0r */
public class Player {
	...
	Timeline.Key.Bone root = new Timeline.Key.Bone(new Vector2(0, 0));
	private final Vector2 position = new Vector2(0, 0), pivot = new Vector2(0, 0);
	...
	private void increaseTime (float delta) {
		time += speed * delta * 60f;
	...
	private void updateRoot () {
		...
		this.root.position.add(position);
		dirty = false;
	}
	 ...
	 * @throws NullVector2erException if no bone exists with the given name */
	 ...
	 * @throws NullVector2erException if no reference for the given bone was found */
	 ...
	 * @throws NullVector2erException if no object exists with the given name */
	 ...
	 * @throws NullVector2erException if no reference for the given object was found */
	 ...
	 * @throws NullVector2erException if no name for the given bone or bject was found */
	 ...
	 * @throws NullVector2erException if no object info for the given bone or bject was found */
	 ...
	 * @throws NullVector2erException if no object info for the given bone or object exists */
	...
	/** Returns whether the given Vector2 at x,y lies inside the box of the given bone or object.
	 * 
	 * @param boneOrObject the bone or object
	 * @param x the x value of the Vector2
	 * @param y the y value of the Vector2
	...
	/** Returns whether the given Vector2 lies inside the box of the given bone or object.
	 * 
	 * @param bone the bone or object
	 * @param Vector2 the Vector2
	 * @return <code>true</code> if the Vector2 lies inside the box of the given bone or object
	 * @throws NullVector2erException if no object info for the given bone or object exists */
	public boolean collidesFor (Bone boneOrObject, Vector2 Vector2) {
		return this.collidesFor(boneOrObject, Vector2.x, Vector2.y);
	}
	 ...
	 * @throws SpriterException if no bone exists of the given name */
	public void setBone (String name, Vector2 position, float angle, Vector2 scale) {
	 ...
	 * @param angle the new angle of the bone
	 * @throws SpriterException if no bone exists of the given name */
	public void setBone (String name, Vector2 position, float angle) {
		Bone b = getBone(name);
	...
	public void setObject (String name, float x, float y, float angle, float scaleX, float scaleY, float pivotX, float pivotY,
		float alpha, int folder, int file) {
	 ...
	 * @throws SpriterException if no object exists of the given name */
	public void setObject (String name, Vector2 position, float angle, Vector2 scale, Vector2 pivot, float alpha,
		FileReference ref) {
	 ...
	 * @param angle the new angle of the object
	 * @throws SpriterException if no object exists of the given name */
	public void setObject (String name, Vector2 position, float angle) {
		Object b = getObject(name);
	 ...
	 * @param position the new position of the object
	 * @throws SpriterException if no object exists of the given name */
	public void setObject (String name, Vector2 position) {
	...
	/** Sets the entity for this player instance. The animation will be switched to the first one of the new entity.
	 * 
	...
	public void setEntity (Entity entity) {
		...
		for (int i = 0; i < maxAnims; i++) {
			...
			key.setObject(new Timeline.Key.Object(new Vector2(0, 0)));
			keyU.setObject(new Timeline.Key.Object(new Vector2(0, 0)));
			tweenedKeys[i] = key;
	 ...
	 * @throws SpriterException if the animation is <code>null</code> or the current animation is not a member of the current set
	 *            entity */
	...
	/** Returns a bounding box for this player. The bounding box is calculated for all bones and object starting from the given
	 * root.
	 * 
	...
	/** Returns the current time. The player will make sure that the current time is always between 0 and {@link Animation#length}.
	 * 
	...
	/** Sets the time for the current time. The player will make sure that the new time will not exceed the time bounds of the
	 * current animation.
	 * 
	...
	public Player scale (float scale) {
		this.root.scale.scl(scale, scale);
		return this;
	}
	 ...
	 * @return this player to enable chained operations */
	public Player translate (Vector2 amount) {
	...
	/** Sets the pivot, i.e. origin, of this player. A pivot at (0,0) means that the origin of the played animation will have the
	 * same one as in Spriter.
	 * 
	 ...
	 * @return this player to enable chained operations */
	public Player setPivot (Vector2 pivot) {
	 ...
	 * @param amount the amount to add
	 * @return this player to enable chained operations */
	public Player translatePivot (Vector2 amount) {
	...
	public static interface PlayerListener {
		...
		/** Gets called if the current animation has reached it's end or it's beginning (depends on the current set
		 * {@link Player#speed}).
		 * 
		 * @param animation the animation which finished. */
		public void animationFinished (Animation animation);

		/** Gets called if the animation of the player gets changed. If {@link Player#setAnimation(Animation)} gets called and the
		 * new animation is the same as the previous one, this method will not be called.
		 * 
		...
		/** Gets called if the mainline key gets changed. If {@link Player#speed} is big enough it can happen that mainline keys
		 * between the previous and the new mainline key will be ignored.
		 * 
	...
	/** An attachment is an abstract object which can be attached to a {@link Player} object. An attachment extends a {@link Bone}
	 * which means that {@link Bone#position}, {@link Bone#scale} and {@link Bone#angle} can be set to change the relative position
	 * to its {@link Attachment#parent} The {@link Player} object will make sure that the attachment will be transformed relative
	 * to its {@link Attachment#parent}.
	 * 
	 * @author Trixt0r */
	public static abstract class Attachment extends Timeline.Key.Bone {

		private Bone parent;
		private final Vector2 positionTemp, scaleTemp;
		...
		public Attachment (Bone parent) {
			this.positionTemp = new Vector2();
			this.scaleTemp = new Vector2();
