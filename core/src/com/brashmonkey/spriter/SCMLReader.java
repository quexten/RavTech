...
/** This class parses a SCML file and creates a {@link Data} instance. If you want to keep track of what is going on during the
 * build process of the objects parsed from the SCML file, you could extend this class and override the load*() methods for pre or
 * post processing. This could be e.g. useful for a loading screen which responds to the current building or parsing state.
 * 
 * @author Trixt0r */
public class SCMLReader {
	...
	protected Data load (String xml) {
		...
		data = new Data(root.get("scml_version"), root.get("generator"), root.get("generator_version"), folders.size(),
			entities.size());
		loadFolders(folders);
	...
	protected void loadFiles (ArrayList<Element> files, Folder folder) {
		for (int j = 0; j < files.size(); j++) {
			...
			File file = new File(f.getInt("id"), f.get("name"), new Dimension(f.getInt("width", 0), f.getInt("height", 0)),
				new Vector2(f.getFloat("pivot_x", 0f), f.getFloat("pivot_y", 1f)));
	...
	protected void loadEntities (ArrayList<Element> entities) {
		for (int i = 0; i < entities.size(); i++) {
			...
			Entity entity = new Entity(e.getInt("id"), e.get("name"), animations.size(), charMaps.size(), infos.size());
	...
	protected void loadAnimations (ArrayList<Element> animations, Entity entity) {
		for (int i = 0; i < animations.size(); i++) {
			...
			Animation animation = new Animation(new Mainline(mainlineKeys.size()), a.getInt("id"), a.get("name"), a.getInt("length"),
	...
	protected void loadMainlineKeys (ArrayList<Element> keys, Mainline main) {
		for (int i = 0; i < main.keys.length; i++) {
			...
			Mainline.Key key = new Mainline.Key(k.getInt("id"), k.getInt("time", 0), curve, boneRefs.size(), objectRefs.size());
			main.addKey(key);
	...
	protected void loadRefs (ArrayList<Element> objectRefs, ArrayList<Element> boneRefs, Mainline.Key key) {
		for (int i = 0; i < boneRefs.size(); i++) {
			...
			BoneRef boneRef = new BoneRef(e.getInt("id"), e.getInt("timeline"), e.getInt("key"),
		...
		for (int i = 0; i < objectRefs.size(); i++) {
			...
			ObjectRef objectRef = new ObjectRef(o.getInt("id"), o.getInt("timeline"), o.getInt("key"),
				key.getBoneRef(o.getInt("parent", -1)), o.getInt("z_index", 0));
	...
	protected void loadTimelines (ArrayList<Element> timelines, Animation animation, Entity entity) {
		for (int i = 0; i < timelines.size(); i++) {
			...
			if (info == null)
				info = new ObjectInfo(name, type, new Dimension(0, 0));
	...
	protected void loadTimelineKeys (ArrayList<Element> keys, Timeline timeline) {
		for (int i = 0; i < keys.size(); i++) {
			...
			Vector2 pivot = new Vector2(obj.getFloat("pivot_x", 0f),
				obj.getFloat("pivot_y", (timeline.objectInfo.type == ObjectType.Bone) ? .5f : 1f));
			...
			if (obj.getName().equals("object")) {
				if (timeline.objectInfo.type == ObjectType.Sprite) {
					...
					pivot = new Vector2(obj.getFloat("pivot_x", f.pivot.x), obj.getFloat("pivot_y", f.pivot.y));
