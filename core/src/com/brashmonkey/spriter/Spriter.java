...
public class Spriter {
	...
	/** Initializes this class with the implemented {@link Loader} class and {@link Drawer} class. Before calling this method make
	 * sure that you have set all necessary dependecies with {@link #setDrawerDependencies(Object...)} and
	 * {@link #setLoaderDependencies(Object...)}. A drawer is created with this method.
	 * 
	...
	/** Creates a new {@link Player} instance based on the given SCML file with the given entity index and the given class
	 * extending a {@link Player}
	 * 
	 * @param scmlFile name of the SCML file
	 * @param entityIndex the index of the entity
	...
	public static Player newPlayer (String scmlFile, int entityIndex, Class<? extends Player> playerClass) {
		...
		try {
			Player player = (Player)ClassReflection.getDeclaredConstructor(playerClass, Entity.class)
				.newInstance(loadedData.get(scmlFile).getEntity(entityIndex));
			players.add(player);
			return player;
		} catch (Exception e) {
			e.printStackTrace();
	...
	/** Updates each created player by this class and immediately draws it. This method should only be called if you want to update
	 * and render on the same thread.
	 * 
	...
	public static void updateAndDraw () {
		...
		for (int i = 0; i < players.size(); i++) {
			players.get(i).update(Gdx.graphics.getDeltaTime());
