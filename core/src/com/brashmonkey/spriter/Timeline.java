...
/** Represents a time line in a Spriter SCML file. A time line holds an {@link #id}, a {@link #name} and at least one {@link Key}.
 * 
 * @author Trixt0r */
public class Timeline {

	public final Key[] keys;
	private int keyVector2er = 0;
	public final int id;
	...
	/** Represents a time line key in a Spriter SCML file. A key holds an {@link #id}, a {@link #time}, a {@link #spin}, an
	 * {@link #object()} and a {@link #curve}.
	 * 
	 * @author Trixt0r */
	public static class Key {
		...
		public static class Bone {
			...
			public Bone (Vector2 position) {
				this(position, new Vector2(1f, 1f), new Vector2(0f, 1f), 0f);
			}
			...
			public void set (Vector2 position, float angle, Vector2 scale, Vector2 pivot) {
			...
			public void map (Bone parent) {
				...
				this.position.rotate(-parent.angle);
				this.position.scl(1f / parent.scale.x, 1f / parent.scale.y);
				this.scale.scl(1f / parent.scale.x, 1f / parent.scale.y);
				this.angle -= parent.angle;
				this.angle *= Math.signum(parent.scale.x) * Math.signum(parent.scale.y);
			}
		...
		/** Represents an object in a Spriter SCML file. A file has the same properties as a bone with an alpha and file extension.
		 * 
		 * @author Trixt0r */
		public static class Object extends Bone {
			...
			public Object (Vector2 position, Vector2 scale, Vector2 pivot, float angle, float alpha, FileReference ref) {
			...
			public Object (Vector2 position) {
				this(position, new Vector2(1f, 1f), new Vector2(0f, 1f), 0f, 1f, new FileReference(-1, -1));
			}

			public Object (Object object) {
				this(object.position.cpy(), object.scale.cpy(), object.pivot.cpy(), object.angle, object.alpha, object.ref);
			}
			...
			public void set (float x, float y, float angle, float scaleX, float scaleY, float pivotX, float pivotY, float alpha,
				int folder, int file) {
			...
			public void set (Vector2 position, float angle, Vector2 scale, Vector2 pivot, float alpha, FileReference fileRef) {
