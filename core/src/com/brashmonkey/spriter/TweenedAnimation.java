...
/** A tweened animation is responsible for updating itself based on two given animations. The values of the two given animations
 * will get interpolated and save in this animation. When tweening two animations, you have to make sure that they have the same
 * structure. The best result is achieved if bones of two different animations are named in the same way. There are still issues
 * with sprites, which are hard to resolve since Spriter does not save them in a useful order or naming convention.
 * 
 * @author Trixt0r */
public class TweenedAnimation extends Animation {

	/** The weight of the interpolation. 0.5f is the default value. Values closer to 0.0f mean the first animation will have more
	 * influence. */
	public float weight = .5f;

	/** Indicates when a sprite should be switched form the first animation object to the second one. A value closer to 0.0f means
	 * that the sprites of the second animation will be drawn. */
	public float spriteThreshold = .5f;

	/** The curve which will tween the animations. The default type of the curve is {@link Curve.Type#Linear}. */
	public final Curve curve;

	/** The entity the animations have be part of. Animations of two different entities can not be tweened. */
	...
	/** Indicates whether to tween sprites or not. Default value is <code>false</code>. Tweening sprites should be only enabled if
	 * they have exactly the same structure. If all animations are bone based and sprites only change their references it is not
	 * recommended to tween sprites. */
	...
	public void update (int time, Bone root) {
		...
		if (base != null) {// TODO: Sprites not working properly because of
			// different timeline naming
			...
			for (BoneRef ref : currentKey.boneRefs) {
				...
				this.unmapTimelineObject(ref.timeline, false,
					(ref.parent != null) ? this.unmappedTweenedKeys[ref.parent.timeline].object() : root);
			}
			/*
			 * for(ObjectRef ref: baseAnim.currentKey.objectRefs){ Timeline timeline =
			 * baseAnim.getTimeline(ref.timeline);//getSimilarTimeline(ref, tempTimelines); if(timeline != null){
			 * //tempTimelines.addLast(timeline); Timeline.Key key = baseAnim.tweenedKeys[timeline.id]; Timeline.Key mappedKey =
			 * baseAnim.mappedTweenedKeys[timeline.id]; Object obj = (Object) key.object();
			 * 
			 * this.tweenedKeys[ref.timeline].active = key.active; ((Object)this.tweenedKeys[ref.timeline].object()).set(obj);
			 * this.mappedTweenedKeys[ref.timeline].active = mappedKey.active; this.unmapTimelineObject(ref.timeline,
			 * true,(ref.parent != null) ? this.mappedTweenedKeys[ref.parent.timeline].object(): root); } }
			 */
			// tempTimelines.clear();
		}
	...
	protected void update (BoneRef ref, Bone root, int time) {
		...
		Timeline t1 = onFirstMainLine() ? anim1.getTimeline(ref.timeline)
		...
		if (t1 != null)
			bone1 = anim1.tweenedKeys[t1.id].object();
		if (t2 != null)
			bone2 = anim2.tweenedKeys[t2.id].object();
		...
		if (this.unmappedTweenedKeys[ref.timeline].active) {
			this.unmapTimelineObject(targetTimeline.id, isObject,
				(ref.parent != null) ? this.unmappedTweenedKeys[ref.parent.timeline].object() : root);
		}
	}
	...
	public void setAnimations (Animation animation1, Animation animation2) {
		...
		if (animation1 == anim1 && animation2 == anim2)
			return;
