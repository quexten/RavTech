...
public class B2DSeparator {
     ...
     *
     * This class has been written by Antoan Angelov. It is designed to work with Erin Catto's Box2D physics library.
     *
     * Everybody can use this software for any purpose, under two restrictions: 1. You cannot claim that you wrote this software.
     ...
     * <ul>
     * <li>This class is specifically for non-convex polygons. If you want to create a convex polygon, you don't need to use this
     * class - Box2D's <code>PolygonShape</code> class allows you to create convex shapes with the <code>setAsArray()</code>/
     * <code>setAsVector()</code> method.</li>
     ...
     * @param fixtureDef A FixtureDef, containing all the properties (friction, density, etc.) which the new fixtures will
     *            inherit.
     * @param verticesVec The vertices of the non-convex polygon, in clockwise order.
     * @param scale <code>[optional]</code> The scale which you use to draw shapes in Box2D. The bigger the scale, the better the
    ...
    public static void separate (Body body, FixtureDef fixtureDef, Vector2[] verticesVec, PolygonCollider polygonCollider) {
        ...
        for (i = 0; i < n; i++) {
            ...
            fixDef.density = fixtureDef.density;
            fixDef.filter.categoryBits = fixtureDef.filter.categoryBits;
            fixDef.filter.groupIndex = fixtureDef.filter.groupIndex;
            fixDef.filter.maskBits = fixtureDef.filter.maskBits;
            fixDef.friction = fixtureDef.friction;
            fixDef.isSensor = fixtureDef.isSensor;
            fixDef.restitution = fixtureDef.restitution;
            fixDef.shape = polyShape;
            polygonCollider.fixtures.add(body.createFixture(fixDef));
        }
    }

    /** Checks whether the vertices in <code>verticesVec</code> can be properly distributed into the new fixtures (more
