...
public class LightGizmo extends Gizmo<Light> {
	...
	public void draw (PolygonShapeRenderer renderer,
		boolean selected) {
		if (component.getLightType() == LightType.ConeLight
			|| component.getLightType() == LightType.PointLight) {
			renderer.setColor(
			...
			((box2dLight.RavChainLight)component.getLight())
			...
			renderer.line(origin.x - 2, origin.y,
				origin.x - 2 + addition.x, origin.y + addition.y);
			renderer.line(origin.x - 1, origin.y,
				origin.x - 1 + addition.x, origin.y + addition.y);
			renderer.line(origin.x + 0, origin.y,
				origin.x + 0 + addition.x, origin.y + addition.y);
			renderer.line(origin.x + 1, origin.y,
				origin.x + 1 + addition.x, origin.y + addition.y);
			renderer.line(origin.x + 2, origin.y,
				origin.x + 2 + addition.x, origin.y + addition.y);
	...
	private void drawCone (PolygonShapeRenderer renderer) {
		...
		boolean isConeLight = component
			.getLightType() == LightType.ConeLight;
		Vector2 origin = new Vector2(
			component.getParent().transform.getPosition().x,
			component.getParent().transform.getPosition().y);
		int indexcount = component.getRayCount()
			+ ((component.getLightType() == LightType.PointLight) ? 1
				: 0);
		...
		for (int i = 0; i < indexcount; i++) {
			float offset = component.getAngle() * 2
	...
	private Vector2 getRayEndpoint (Vector2 origin, float degrees) {
		return new Vector2(
			origin.x + (float)Math.cos(Math.toRadians(
				component.getParent().transform.getRotation() + degrees))
			* component.getDistance(),
			origin.y + (float)Math.sin(Math.toRadians(
				component.getParent().transform.getRotation() + degrees))
				* component.getDistance());
		...
		switch (eventType) {
			case EventType.MouseMoved:
				float coneDst = Math.abs(
					origin.dst(worldPosition) - component.getDistance());
				float rayDst = GeometryUtils.isInBoundingBox(origin,
					getRayEndpoint(origin, component.getAngle()),
					worldPosition, 1)
			...
			case EventType.MouseDrag:
				if (!raySelected)
					...
					if (angle < 0)
						angle += 360;
					angle = angle > 180 ? 180 - Math.abs(angle - 180)
						: angle;
