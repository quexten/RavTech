...
public class BoxColliderGizmo extends Gizmo<BoxCollider> {
	...
	public void draw (PolygonShapeRenderer renderer,
		...
		Vector2 middlePosition = component.getParent().transform
			.getPosition().cpy()
			.add(new Vector2(component.x, component.y).rotate(rotation));
		Vector2 tl = middlePosition.cpy()
			.add(new Vector2(component.width / 2, component.height / 2)
				.rotate(+rotation));
		Vector2 tr = middlePosition.cpy()
			.add(new Vector2(component.width / 2, -component.height / 2)
				.rotate(+rotation));
		Vector2 br = middlePosition.cpy()
			.sub(new Vector2(component.width / 2, component.height / 2)
				.rotate(+rotation));
	...
	public float input (float x, float y, int button, int eventType) {
		...
		Vector2 middlePosition = component.getParent().transform
			.getPosition().cpy().sub(
				new Vector2(-component.x, -component.y).rotate(rotation));
		Vector2 mousePosition = new Vector2(x, y);
		switch (eventType) {
			case EventType.MouseMoved:
				Vector2 tl = middlePosition.cpy().sub(
					new Vector2(component.width / 2, component.height / 2)
						.rotate(+rotation));
				Vector2 tr = middlePosition.cpy().sub(
					new Vector2(component.width / 2, -component.height / 2)
						.rotate(+rotation));
				Vector2 br = middlePosition.cpy().add(
					new Vector2(component.width / 2, component.height / 2)
						.rotate(+rotation));
				Vector2 bl = middlePosition.cpy().add(
					new Vector2(component.width / 2, -component.height / 2)
						.rotate(+rotation));
			...
			case EventType.MouseDown:
				oldPosition = component.getParent().transform
					.getPosition().cpy()
					.sub(new Vector2(-component.x * 2, -component.y * 2)
						.rotate(rotation));
				middlePosition = oldPosition;
				oldPosition = component.getParent().transform
					.getPosition().cpy()
					.sub(new Vector2(-component.x, -component.y)
						.rotate(rotation));
				trueOldPosition = component.getParent().transform
					.getPosition().cpy()
					.sub(new Vector2(-component.x * 2, -component.y * 2)
						.rotate(0));
				oldBounds = new Vector2(component.width,
					component.height);
				isGrabbed = true;
					...
					switch (grabbedPoint) {
						...
						case 8:
							Vector2 subPosition = mousePosition
								.sub(component.getParent().transform
									.getPosition().cpy());
							component.x = subPosition.x / component.width
								* 2;
							component.y = subPosition.y / component.height
								* 2;
							break;
					}
				return -1f;
	...
	private void changeWidth (float width, boolean changeRight) {
		...
		component.x = trueOldPosition.cpy().add(addPosition)
			.sub(component.getParent().transform.getPosition().cpy()).x
			/ 2;
	...
	private void changeHeight (float height, boolean changeTop) {
		...
		component.y = trueOldPosition.cpy().add(addPosition)
			.sub(component.getParent().transform.getPosition().cpy()).y
			/ 2;
