    ...
    public float input (int button, int eventType) {
        ...
        switch (eventType) {
            case EventType.MouseMoved:
                for (int i = 0; i < polygonCollider.vertecies.size; i++)
                    if (i < polygonCollider.vertecies.size) {
                        Vector2 firstPoint = polygonCollider.getParent().transform
                            .getPosition(polygonCollider.vertecies.get(i).cpy());
                        Vector2 secondPoint = polygonCollider.getParent().transform
                            .getPosition(i < polygonCollider.vertecies.size - 1 ? polygonCollider.vertecies.get(i + 1).cpy()
                                : polygonCollider.vertecies.get(0).cpy());
                        if (GeometryUtils.isPointNearLine(firstPoint, secondPoint, mousePosition, closestDst)) {
                            float dst = GeometryUtils.dstFromLine(firstPoint, secondPoint, mousePosition);
                            if (dst < 0.3f) {
                                selectedLine = i;
            ...
            case EventType.MouseDrag:
                if (isGrabbed) if (grabbedPoint != null)
