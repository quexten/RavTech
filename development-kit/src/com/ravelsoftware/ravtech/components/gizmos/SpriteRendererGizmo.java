...
public class SpriteRendererGizmo extends Gizmo {
    ...
    public float input (int button, int eventType) {
        ...
        switch (eventType) {
            ...
            case EventType.MouseDown:
                ...
                oldPosition = spriteRenderer.getParent().transform.getPosition()
                    .sub(new Vector2(spriteRenderer.originX * (spriteRenderer.width / 2),
                        spriteRenderer.originY * (spriteRenderer.height / 2)).rotate(rotation));
                middlePosition = oldPosition;
                oldPosition = spriteRenderer.getParent().transform.getPosition()
                    .sub(new Vector2(spriteRenderer.originX * (spriteRenderer.width / 2),
                        spriteRenderer.originY * (spriteRenderer.height / 2)).rotate(rotation));
                trueOldPosition = spriteRenderer.getParent().transform.getPosition().sub(
                    new Vector2(spriteRenderer.originX * spriteRenderer.width, spriteRenderer.originY * spriteRenderer.height)
                        .rotate(0));
            ...
            case EventType.MouseDrag:
                if (isGrabbed) switch (grabbedPoint) {
                    case 0: // tl
                        changeBounds(
                            mousePosition.cpy().sub(oldPosition).rotate(-spriteRenderer.getParent().transform.getRotation()).x,
                            mousePosition.cpy().sub(oldPosition).rotate(-spriteRenderer.getParent().transform.getRotation()).y,
                            false, false);
                        break;
                    case 1: // tr
                        changeBounds(
                    ...
                    case 3: // bl
                        ...
                        break;
                    case 4: // t
                        changeHeight(mousePosition.sub(oldPosition).rotate(-spriteRenderer.getParent().transform.getRotation()).y,
                            true);
                        break;
                    case 5: // r
                        changeWidth(mousePosition.sub(oldPosition).rotate(-spriteRenderer.getParent().transform.getRotation()).x,
                            true);
                        break;
                    case 6: // b
                        changeHeight(mousePosition.sub(oldPosition).rotate(-spriteRenderer.getParent().transform.getRotation()).y,
    ...
    private void changeBounds (float width, float height, boolean changeRight, boolean changeTop) {
        ...
        spriteRenderer.originX = -trueOldPosition.cpy().add(addPosition)
            .sub(spriteRenderer.getParent().transform.getPosition()).x;
        spriteRenderer.originY = trueOldPosition.cpy().add(addPosition).sub(spriteRenderer.getParent().transform.getPosition()).y;
        height = (changeTop ? 1 : -1f) * (height - oldBounds.y * 0.5f) + oldBounds.y * (changeTop ? 1 : 0);
