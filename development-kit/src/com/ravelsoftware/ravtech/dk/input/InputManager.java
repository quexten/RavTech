...
public class InputManager implements InputProcessor {
	...
	public boolean scrolled (int amount) {
		/*
		 * hasToLerp = true; float lastzoom = RavTech.sceneHandler.worldCamera.zoom; Vector2 lastposition = new
		 * Vector2(RavTech.sceneHandler.worldCamera.position.x, RavTech.sceneHandler.worldCamera.position.y); targetZoom += amount *
		 * RavTech.sceneHandler.worldCamera.zoom * 0.5; if (targetZoom < 0) targetZoom = 0.0001f; targetPosition =
		 * RavTech.input.getWorldPosition(); targetPosition = targetPosition.add(lastposition.sub(targetPosition).scl(targetZoom /
		 * lastzoom)); if (!timerStarted) { Timer.schedule(new Task() {
		 * 
		 * @Override public void run () { if (hasToLerp) { RavTech.sceneHandler.worldCamera.zoom += 0.16f * (targetZoom -
		 * RavTech.sceneHandler.worldCamera.zoom); RavTech.sceneHandler.worldCamera.position.lerp(new Vector3(targetPosition.x,
		 * targetPosition.y, 0), 0.16f); } if (Math.abs(targetZoom - RavTech.sceneHandler.worldCamera.zoom) < 0.00001f) hasToLerp =
		 * false; } }, 0, 0.016f); timerStarted = true; }
		 */
		return false;
	}
	...
	public boolean touchDown (int screenX, int screenY, int pointer, int button) {
		/*
		 * RavTechDKUtil.inspectorChanged(); RavTechDK.gizmoHandler.input(button, EventType.MouseDown); Vector3 unprojectedPosition
		 * = RavTech.sceneHandler.worldCamera.unproject(new Vector3(screenX, screenY, 0)); if (button == Buttons.LEFT) {
		 * selectionAlpha = 0.3f; dragStartPosition = new Vector2(unprojectedPosition.x, unprojectedPosition.y); dragCurrentPosition
		 * = dragStartPosition.cpy(); } else touchDownCoords = new Vector2(unprojectedPosition.x, unprojectedPosition.y);
		 */
		return false;
	}
	...
	public boolean touchDragged (int screenX, int screenY, int pointer) {
		...
		if (Gdx.input.isButtonPressed(Buttons.RIGHT)) {
			/*
			 * int screenCenterWidth = Gdx.graphics.getWidth() / 2; int screenCenterHeight = Gdx.graphics.getHeight() / 2; int
			 * screenDiffX = screenCenterWidth - screenX; int screenDiffY = screenCenterHeight + screenY - Gdx.graphics.getHeight();
