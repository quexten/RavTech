...
public class ApkPreparationStep extends PackageStep {
	...
	@Override
	public void run () {
		PackageStep firstStep = new CopyStep(buildReporterDialog,
			getIcon("hdpi"), getIconDst("hdpi"));
		firstStep
			.setNextStep(new CopyStep(buildReporterDialog,
				getIcon("xhdpi"), getIconDst("xhdpi")))
			.setNextStep(new CopyStep(buildReporterDialog,
				getIcon("xxhdpi"), getIconDst("xxhdpi")))
			.setNextStep(new CopyStep(buildReporterDialog,
			...
			.setNextStep(new CreateFileStep(buildReporterDialog,
				...
				Gdx.files
					.absolute(System.getProperty("user.dir")
						+ "/build-templates/android/build.gradle")
					.readString()
					.replaceAll("APP_ID",
			...
			.setNextStep(new CreateFileStep(buildReporterDialog,
				...
				Gdx.files
					.absolute(System.getProperty("user.dir")
						+ "/build-templates/android/AndroidManifest.xml")
					.readString()
					.replaceAll("APP_ID", RavTechDK.project.appId)
					.replaceAll("VERSION_CODE",
			...
			.setNextStep(new CreateFileStep(buildReporterDialog,
				...
				Gdx.files
					.absolute(System.getProperty("user.dir")
						+ "/build-templates/android/strings.xml")
					.readString()
					.replaceAll("APP_NAME", RavTechDK.project.appName)
					.getBytes()));
		firstStep.run();
		executeNext();
	}
	...
	FileHandle getIcon (String name) {
		return RavTechDK.projectHandle.child("icons").child("android")
			.child(name + ".png");
	}

	FileHandle getIconDst (String name) {
		return Gdx.files.absolute(System.getProperty("user.dir")
			+ "/builder/android/res/drawable-" + name
			+ "/ic_launcher.png");
	}
