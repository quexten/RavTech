...
/** Cell renderer for file names in the Details View when Substance is not installed.
 *
...
class DetailsViewFileNameRenderer extends DefaultTableCellRenderer {
     ...
     *
     */
    private static final long serialVersionUID = 8430201852985541712L;
    ...
    public Component getTableCellRendererComponent (JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
        ...
        setIcon(info.icon);
        if (!isSelected) if (chooser.getShowHiddenFiles() && file.isHidden())
    ...
    public void paintComponent (Graphics g) {
        ...
        String clippedText = SwingUtilities.layoutCompoundLabel(this, fm, text, icon, getVerticalAlignment(),
            getHorizontalAlignment(), getVerticalTextPosition(), getHorizontalTextPosition(), paintViewR, paintIconR, paintTextR,
            getIconTextGap());
        if (icon != null) icon.paintIcon(this, g, paintIconR.x, paintIconR.y);
        if (text != null) {
            ...
            if (isAlreadyOpened && chooser.getStyleOpenFiles()) g.drawLine(textX, textY + 2, textX + paintTextR.width, textY + 2);
            if (old != null) ((Graphics2D)g).addRenderingHints(old);
        }
    }
}
