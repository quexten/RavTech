...
public class UpdaterWidget extends VisWindow {
	...
	public UpdaterWidget () {
		...
		Entries<String, Updater> entries = UpdateManager.getUpdaters().iterator();
		while (entries.hasNext) {
			Entry<String, Updater> entry = entries.next();
			UpdaterEntry updaterEntry = new UpdaterEntry(entry.key, entry.value);
	...
	public class UpdaterEntry extends VisTable {
		...
		public UpdaterEntry (String title, final Updater updater) {
			...
			VisLabel titleLabel = new VisLabel(title,
				new VisLabel.LabelStyle((BitmapFont)RavTechDK.editorAssetManager.get("fonts/OpenSansBold.fnt"), Color.BLACK));
			add(titleLabel).padLeft(10).align(Align.left);
			row();

			add(new VisLabel(updater.getDescription())).align(Align.left).padLeft(15);
			row();

			VisTable subLineTable = new VisTable();
			versionLabel = new VisLabel("Version: " + updater.currentVersion() + "/" + "");
			versionLabel.setStyle(new LabelStyle(versionLabel.getStyle()));
